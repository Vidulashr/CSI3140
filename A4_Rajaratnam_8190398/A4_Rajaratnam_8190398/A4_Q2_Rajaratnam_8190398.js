...
//Checks whether given list is in order form lowest to highest to see if game is won
function wonGame(list){
    var result = true;
        ...
        if (pressed.innerText == ""){
            ...
            if (secondpress.innerText == ""){
                ...
                if (Adjacent(pressed, secondpress)==false){
                    ...
                    pressed = null;
                    secondpress = null;
                    //If game won, ask to play again 
                    if (wonGame(shuffle)){
                        var again = prompt("YOU WON! Do you want to play again? [y/n]");
                        while ((again != "y")&&(again != "n")){
                            var again = prompt("Please enter valid key! Do you want to play again? [y/n]");
                        }
                        if (again == "n"){
                            alert("Thanks for Playing!");
                        }
                        else{
                            shuffle = [] //delete elements in list
                            //Shuffle list again
                            for (i = 0; i < tiles.length; i++) {
                                var numbertoadd = getRandomBetween(1,17);
                                while (Present(numbertoadd,shuffle)){
                            ...
                            populate(shuffle); //populate again
                        }
                    }
                }
            }
